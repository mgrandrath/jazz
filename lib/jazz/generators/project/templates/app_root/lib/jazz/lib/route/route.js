...
Jazz.Route.add = function(path, action){
...
Jazz.Route.navigate = function(){
    ...
    function(action, route){
      ...
      for (var part in parts){
        ...
        if(parts[part].indexOf(":") != -1){
          parts[part] = current_url_parts[part]
        }
        
      }
      
      if(parts.join('/') == current_url_parts.join('/')){
          ignore = false;
      }
      
      if(!ignore){
        best_matching_route.url = route.split('/');
      }
  ...
  for (var part in best_matching_route.url){
    if(best_matching_route.url[part].search(":") >= 0){
      window.params[best_matching_route.url[part].replace( /[:]/, '' )] = current_url_parts[part];
